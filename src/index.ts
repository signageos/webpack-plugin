	...
	public apply(compiler: webpack.Compiler) {
		...
		compiler.hooks.done.tap('SignageOSPlugin', async (stats) => {
			...
			if (dev && appletOptions) {
				...
				console.info(`Applet ${appletOptions.appletUid} v${appletOptions.appletVersion} built into ${build.packageArchivePath}`);
...
async function getCurrentOrganizationUid() {
	const cliOptions = cliArgs([NO_DEFAULT_ORGANIZATION_OPTION, ORGANIZATION_UID_OPTION], { partial: true }) as CommandLineOptions<
...
<script>
	...
	window.__SOS_AUTO_VERIFICATION.organizationUid = ${JSON.stringify(envVars.organizationUid)};
		...
		server.listen(defaultPort, () => {
			...
			console.info(`Emulator is running at ${chalk.blue(chalk.bold(emulatorUrl))}`);
		});
		...
		app.use(APPLET_DIRECTORY_PATH, (req: express.Request, res: express.Response, next: () => void) => {
			...
			} else if (typeof lastCompilationAssets[relativeFilePath] !== 'undefined') {
